...
<Callout type="info">
  The **Split Provider** is configured in the Store edit form under the retrieval settings. Use `QA` for FAQ-style content where question/answer pairs should stay in the same chunk.
</Callout>

![Store split provider configuration](/img/docs/openagent/split-provider.png)
...
<Callout type="info">
  **Forbidden Words** are configured in the lower section of the Store settings, alongside other content-safety controls.
</Callout>

![Store forbidden words configuration](/img/docs/openagent/forbidden-words.png)
...
<Callout type="info">
  **Child Stores** are selected in the Store configuration form and let one Store search across the knowledge bases of several specialized Stores.
</Callout>

![Store child stores selection](/img/docs/openagent/child-stores.png)
